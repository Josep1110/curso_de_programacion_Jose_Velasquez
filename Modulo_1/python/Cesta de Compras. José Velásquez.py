...
def agregar_producto(cesta):
    "Añade un nuevo producto a la cesta de compras."
    nombre = input("Introduce el nombre del producto: ").strip()
    while True:
        try:
            precio = float(input(f"Introduce el precio de '{nombre}': "))
            if precio <= 0:
                print("El precio no puede ser igual o menor a cero. Inténtalo de nuevo.")
            else:
                break
        except ValueError:
    ...
    while True:
        try:
            ...
            if cantidad <= 0:
                print("La cantidad no puede ser igual o menor a cero. Inténtalo de nuevo.")
            else:
                break
        except ValueError:
